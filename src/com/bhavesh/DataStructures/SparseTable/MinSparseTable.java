/*
 * Implementation of a min sparse table
 *
 * Construction complexity: O(nlog(n)), query complexity: O(1)
 */
...
public class MinSparseTable {
    ...
    private void init(long[] values) {
        ...
        // Build sparse table containing the values of the previous intervals
        for (int p = 1; p <= P; p++) {
    ...
    private long queryMin(int l, int r) {
        ...
        return Math.min(cache[p][l], cache[p][r - k + 1]);
    }
    ...
    public int queryMinIndex(int l, int r) {
        ...
        long rightInterval = cache[p][r - k + 1];
        if (leftInterval <= rightInterval) {
            return it[p][l];
        }
        else {
