/*
 * Implementation of finding the Lowest Common Ancestor (LCA) of a tree.
 * This implementation first finds an Euler tour from the root node which
 * visits all nodes in the tree. The node height values obtained from the Euler tour
 * can then be used in combination with a sparse table to find the LCA in O(1)
 *
 * Time Complexity : O(1) : queries, O(n*log(n)) : pre-processing
...
public class LowestCommonAncestor {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("LCA of 13 and 14 = " + lca);
        if (lca.index() != 2) {
        ...
        // LCA of 9 and 11 = 0
        lca = solver.lca(9, 11);
        System.out.println("LCA of 9 and 11 = " + lca);
        if (lca.index() != 0) {
        ...
        System.out.println("LCA of 12 and 12 = " + lca);
        if (lca.index() != 12) {
    ...
    public static class TreeNode {
        ...
        private static TreeNode buildTree(List<List<Integer>> graph, TreeNode node) {
            ...
            for (int neighbour : graph.get(node.index())) {
                // Ignore adding an edge back to the parent
                if (node.parent() != null && neighbour == node.parent().index())
                    continue;
    ...
    // This map keeps track of the last occurrence of a TreeNode in the
    // Euler tour for easy indexing
    private int[] last;

    // Sparse table which can efficiently do Range Minimum Queries (RMQs)
    ...
    // Find the lowest common ancestor of the nodes with `index1` and `index2`
        ...
        private void init(long[] v) {
            ...
            for (int p = 1; p <= P; p++) {
                for (int i = 0; i + (1 << p) <= n; i++) {
                    long leftInterval = cache[p - 1][i];
                    long rightInterval = cache[p - 1][i + (1 << (p - 1))];
                    cache[p][i] = Math.min(leftInterval, rightInterval);
